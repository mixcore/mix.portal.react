...
class FetchClient {
    ...
    private async handleResponse(response: Response) {
        if (response.status === 401 && !this.refreshAttempted) {
            ...
            try {
                ...
                const refreshResponse = await fetch(`${API_URL}/rest/security/refresh-token`, {
                    method: 'POST',
                    headers: {
                ...
                if (refreshResponse.ok && refreshData.success) {
                    ...
                    return this.fetchWithInterceptor(originalRequest.url, originalRequest.options);
                } else {
                    this.logout();
    ...
    private async fetchWithInterceptor(url: string, options: RequestInit = {}): Promise<any> {
        try {
